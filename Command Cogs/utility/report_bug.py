...
def setup(Bot):
    ...
    async def _report_bug(ctx, *, text):
        ...
        if timedelta < cooldowns_json["bugreport"]:
            ...
            await ctx.send(embed=general_utils.error_embed(Bot, ctx, True, f"The length of your bug report ({len(text)} characters) can't exceed 2^11 characters, please shorten the report and try again (the cooldown has not been refreshed)."))
            return
