...
def setup(Bot):
    ...
    async def _start(ctx):
        ...
        try:
            ...
            await msg.edit(embed=general_utils.error_embed(Bot, ctx, False, f"Sorry, but you'll need friends to play this, cah games need to have at least 2 players to start."))
            return
        except Bot.cah.errors.NotInAGame:
            await msg.edit(embed=general_utils.error_embed(Bot, ctx, False, f"You arent in a game yet!"))
            return
        except Bot.cah.errors.NotGameOwner:
            await msg.edit(embed=general_utils.error_embed(Bot, ctx, False, f"You arent the owner of your current game, you need to be the owner to use this command."))
            return
