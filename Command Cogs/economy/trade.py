            ...
            for key, value in losing.items():
                if key.lower() in ["simoleons", "coins", "coin", "simoleon", "money", "dollars"]: #deal with coins
                    ...
                    if int(value) > user_items[item_reference[key.lower()]]:
                        error_embed.title = f"{whom.name} does not have enough {key}{general_utils.item_plural(item_json[item_reference[key.lower()]], 2)}!"+(f" They were giving {value} despite only having {user_items[item_reference[key.lower()]]}." if is_invisible else '')
                        await ctx.send(embed=error_embed)
                        return False
                    if int(value) < 1:
                        error_embed.title = f"You cannot specify an amount less than 1 when trading. ({whom.name} was giving {value} {key}{general_utils.item_plural(item_json[item_reference[key.lower()]], value)})"
                        await ctx.send(embed=error_embed)
                        return False
